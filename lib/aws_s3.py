...
bucket_name = "project1027657data"
...
def archive_landing_object(filename: str, dataset: str) -> bool:
    try:
        ...
        copy_source = {'Bucket': bucket_name, 'Key': f"data_with_pyspark/landing/{dataset}/{filename}"}
        response = client.copy_object(Bucket = bucket_name, CopySource = copy_source, Key = f"data_with_pyspark/archive/{dataset}/{filename}")
        if response['ResponseMetadata']['HTTPStatusCode'] == 200:
            client.delete_object(Bucket = bucket_name, Key = f"data_with_pyspark/landing/{dataset}/{filename}")
            return True
        else:
            return False
